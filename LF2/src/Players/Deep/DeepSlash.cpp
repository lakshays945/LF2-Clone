...
const std::vector<RealVector2D> EndLocations = { {0,85},{82,85},{164,85}, {246,85} };
const std::vector <double> EndTimes = { 0.1,0.2,0.3,0.4 };

DeepSlash::DeepSlash(){
	IsActive = false;
	if (SlashTexFile.getSize() == sf::Vector2u(0, 0)) {
		SlashTexFile.loadFromFile("Resource/DeepSlash.png");
	}
	InitialSheet.AssignTextures(SlashTexFile, InAirLocations, InAirTimes, 75, 75);
	InAirSheet.AssignTextures(SlashTexFile, InAirLocations, InAirTimes, 75, 75);
	FastSheet.AssignTextures(SlashTexFile, InAirLocations, InAirTimes, 75, 75);
	EndSheet.AssignTextures(SlashTexFile, EndLocations, EndTimes, 75, 75);
...
void DeepSlash::Animate(sf::RenderWindow& window, const double dt){
	if (!IsActive) {
		return;
	}
	...
	TotalTime += dt;
	if (TotalTime > 0.5 && CurrentSheet != &EndSheet) {
...
void DeepSlash::OnCollision(int otherID, int selfID){
...
void DeepSlash::OnCollisionExit(int otherID, int selfID)
{
}

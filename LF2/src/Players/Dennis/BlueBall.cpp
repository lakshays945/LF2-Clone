...
void BlueBall::AssignParent(GameObject* parent) {
	Parent = parent;
	Effect_Manager = parent->Effect_Manager;
}

void BlueBall::Animate(sf::RenderWindow& window, const double dt) {
	if (!IsActive) {
		return;
	}
...
void BlueBall::Instantiate(RealVector2D velocity) {
	Position = Parent->Position;
	InAirSheet.Time = 0;
	EndSheet.Time = 0;
	IsActive = true;
...
void BlueBall::OnCollision(int otherID, int selfID) {

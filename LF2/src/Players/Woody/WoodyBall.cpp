...
WoodyBall::WoodyBall() {
	IsActive = false;
	if (WoodyBallTexFile.getSize() == sf::Vector2u(0, 0)) {
		WoodyBallTexFile.loadFromFile("Resource/WoodyBall.png");
	}
	InitialSheet.AssignTextures(WoodyBallTexFile, InAirLocations, InAirTimes, 75, 75);
	InAirSheet.AssignTextures(WoodyBallTexFile, InAirLocations, InAirTimes, 75, 75);
	FastSheet.AssignTextures(WoodyBallTexFile, InAirLocations, InAirTimes, 75, 75);
	EndSheet.AssignTextures(WoodyBallTexFile, EndLocations, EndTimes, 75, 75);
...
void WoodyBall::Animate(sf::RenderWindow& window, const double dt){
	if (!IsActive) {
		return;
	}
...
void WoodyBall::OnCollision(int otherID, int selfID){
...
void WoodyBall::OnCollisionExit(int otherID, int selfID)
{
}
